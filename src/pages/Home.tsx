...
import { Heart, MessageCircle, TrendingUp, Users, Zap, X, Send, Calendar, MapPin, Coffee, Mic, HeartHandshake, MailQuestion, ShieldCheck } from "lucide-react";
...
// DATA JADWAL KEGIATAN ABANG (Bisa diedit-edit isinya)
const upcomingEvents = [
  { title: "Podcast GenZTalk Ep. 1", date: "20 Mar 2026", type: "Live YouTube", icon: Mic, desc: "Ngobrol santai bahas Hustle Culture vs Burnout." },
  { title: "Kopi Darat & Diskusi", date: "25 Mar 2026", type: "Cafe Soreang", icon: Coffee, desc: "Nongkrong santai sambil bahas isu sosial terkini." },
  { title: "GenZTalk Berbagi", date: "02 Apr 2026", type: "Bakti Sosial", icon: HeartHandshake, desc: "Bagi-bagi takjil dan sembako ke jalanan." }
];
...
export default function Home() {
  ...
  const fetchOpinions = async () => {
    setLoading(true);
    const { data, error } = await supabase.from('opinions').select('*, likes(user_email), comments(id)').order('createdat', { ascending: false });
    if (data) {
      const currentUser = (await supabase.auth.getSession()).data.session?.user;
      const formattedData = data.map((item: any) => ({
        id: item.id, title: item.title, content: item.content,
        author: { username: item.author_username, avatar: item.author_avatar || IMAGES.COMMUNITY_1 },
        category: item.category, createdAt: new Date(item.createdat),
  ...
  useEffect(() => {
    // Cek apakah mode admin aktif di Local Storage
  ...
  useEffect(() => {
    fetchOpinions();
    if (isAdmin) fetchFeedbacks(); // Tarik data aduan rahasia khusus Admin
  }, [user, isAdmin]);
  ...
  const handleSubmitFeedback = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('feedbacks').insert([{
      author_name: user.user_metadata?.full_name || 'Anonim',
      author_email: user.email,
      content: feedbackContent
    }]);
    setIsSubmittingFeedback(false);
    if (!error) {
      alert("Aduan berhasil dikirim! Tenang aja, pesan ini rahasia dan cuma admin yang bisa baca.");
      setFeedbackContent("");
  ...
  return (
    <Layout>
      {/* ... [KODE MODAL OPINI & KOMENTAR TETAP SAMA, SAYA SKIP BIAR RINGKAS] ... */}
      <AnimatePresence>
        {isWriting && (
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="bg-background border-2 border-primary/30 rounded-2xl p-6 w-full max-w-2xl shadow-[0_0_40px_rgba(168,85,247,0.3)] relative">
              ...
              <form onSubmit={handleSubmitOpinion} className="space-y-4">
                <div>
                  ...
                  <input type="text" required value={newTitle} onChange={(e) => setNewTitle(e.target.value)} className="w-full bg-card/50 border border-primary/20 rounded-lg px-4 py-3 text-foreground focus:outline-none focus:border-primary transition-all" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-muted-foreground mb-1">Kategori</label>
                  <select value={newCategory} onChange={(e) => setNewCategory(e.target.value)} className="w-full bg-card/50 border border-primary/20 rounded-lg px-4 py-3 text-foreground focus:outline-none focus:border-primary transition-all">
                    <option value="Karir">Karir & Kuliah</option><option value="Mental Health">Mental Health</option><option value="Relationship">Relationship</option><option value="Sosial">Isu Sosial</option>
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-muted-foreground mb-1">Isi Opini</label>
                  <textarea required rows={6} value={newContent} onChange={(e) => setNewContent(e.target.value)} className="w-full bg-card/50 border border-primary/20 rounded-lg px-4 py-3 text-foreground focus:outline-none focus:border-primary transition-all resize-none" />
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 max-w-6xl mx-auto">
            {logoPhilosophy.map((item, index) => (
              <motion.div key={index} 
                initial={{ opacity: 0, x: index % 2 === 0 ? -50 : 50 }} // <--- Animasi Kiri & Kanan
      ...
      <section id="events" className="py-24 relative bg-background/30 border-t border-primary/10 overflow-hidden">
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.5 }} className="text-center mb-16 max-w-3xl mx-auto">
            ...
            <p className="text-muted-foreground text-lg">Ikuti kegiatan seru kami secara offline maupun online!</p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
            {upcomingEvents.map((event, index) => (
              ...
              >
                <Card className="p-6 h-full bg-card/50 border border-primary/20 hover:border-accent/50 transition-colors group">
                  <div className="flex items-center gap-4 mb-4 pb-4 border-b border-primary/10">
                    <div className="w-12 h-12 rounded-full bg-accent/20 flex items-center justify-center text-accent group-hover:bg-accent group-hover:text-black transition-all">
                      <event.icon size={24} />
                    </div>
                    <div>
                      <h3 className="font-bold text-lg text-foreground">{event.title}</h3>
                      <div className="flex items-center gap-2 text-xs text-muted-foreground mt-1">
                        <Calendar size={12} /> {event.date} • <MapPin size={12} className="ml-1"/> {event.type}
                      </div>
                    </div>
                  </div>
                ...
                <motion.div key={opinion.id} initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.1 }}>
                  <Card className="h-full backdrop-blur-xl bg-card/40 border-2 border-primary/20 p-6 flex flex-col hover:border-primary/50 transition-colors">
                    ...
                    <div className="flex items-center gap-6 pt-4 border-t border-border/50">
                      <button onClick={() => handleLike(opinion.id, opinion.isLikedByMe)} className={`flex items-center gap-2 transition-colors group/btn ${opinion.isLikedByMe ? 'text-red-500' : 'text-muted-foreground hover:text-red-500'}`}>
                      ...
                      <button onClick={() => openComments(opinion)} className="flex items-center gap-2 text-muted-foreground hover:text-accent transition-colors group/btn">
      ...
      {/* SECTION BARU: KOTAK ADUAN (Rahasia) */}
          ...
          <motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.6 }}>
            <Card className="p-8 md:p-12 bg-card/40 border-2 border-primary/30 backdrop-blur-md relative overflow-hidden">
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <h3 className="text-2xl font-bold text-foreground">Kotak Suara Pribadi</h3>
                  <p className="text-sm text-muted-foreground">Punya keluhan, saran, atau aduan? Sampaikan di sini. 100% Rahasia.</p>
                </div>
              </div>
              <form onSubmit={handleSubmitFeedback}>
                <textarea 
                  required rows={4} value={feedbackContent} onChange={(e) => setFeedbackContent(e.target.value)}
                  placeholder={user ? "Ketik pesan rahasiamu ke Admin di sini..." : "Silakan Login dulu buat kirim pesan Bang..."}
                ...
                />
                <Button type="submit" disabled={!user || isSubmittingFeedback} className="w-full bg-primary hover:bg-primary/90 text-white font-bold py-6">
                  {isSubmittingFeedback ? "Mengirim..." : "Kirim Pesan ke Admin"}
                </Button>
              </form>
            </Card>
          </motion.div>
      ...
      {/* DASHBOARD KHUSUS ADMIN (Cuma Muncul Kalau Mode Admin Aktif) */}
      {isAdmin && (
          ...
          <div className="container mx-auto px-4 max-w-5xl">
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              <div className="flex items-center gap-3 mb-8">
                ...
                <h2 className="text-3xl font-bold text-foreground">Dapur Admin: Daftar Aduan</h2>
              </div>
              
              <div className="space-y-4">
                {adminFeedbacks.length === 0 ? (
                  <p className="text-muted-foreground">Belum ada aduan masuk nih, Bang Admin.</p>
                ) : (
                  adminFeedbacks.map((fb) => (
                    <Card key={fb.id} className="p-6 bg-card border-l-4 border-l-red-500 shadow-md">
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <p className="font-bold text-foreground">{fb.author_name}</p>
                          <p className="text-xs text-muted-foreground">{fb.author_email}</p>
                        </div>
                        <span className="text-xs text-muted-foreground bg-background px-2 py-1 rounded-full">{formatDate(new Date(fb.created_at))}</span>
                      </div>
        ...
        <section id="membership" className="py-24 relative overflow-hidden">
           ...
           <div className="container mx-auto px-4 text-center">
            ...
            <Card className="max-w-5xl mx-auto bg-card/40 border-2 border-accent/30 p-8 md:p-12">
              <Button onClick={handleLogin} size="lg" className="text-lg px-12 py-6 bg-gradient-to-r from-accent to-primary mt-8">Daftar Sekarang</Button>
            </Card>
          </div>
        </section>
      )}
